...
export default class LoadMore extends Component {
  ...
  componentDidMount() {
    ...
    this.fetchData(responseJson => {
      ...
      console.log('RESPOSE@@@@: ' + JSON.stringify(data.length))
      this.setState({
              ...
              <View style={styles.listItem}>
                <View style={styles.imageWrapper}>
                  <Image
                    ...
                    source={{
                      uri: rowData.data.icon_img === '' || rowData.data.icon_img === null ? 'https://via.placeholder.com/70x70.jpg' : rowData.data.icon_img
                    }}
                  />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.title}>{rowData.data.display_name}</Text>
                  <Text style={styles.subtitle}>{rowData.data.public_description}</Text>
                </View>
              </View>
            )
          }}
